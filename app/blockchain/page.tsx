...
const HashBlock = () => {
    const [blocks, setBlocks] = useState([
        { number: 1, nonce: "89565", data: "", hash: "0000", prevHash: `${firstprev}`, isMined: false, isBlinking: false },
        { number: 2, nonce: "89565", data: "", hash: "0000", prevHash: "", isMined: false, isBlinking: false },
        { number: 3, nonce: "89565", data: "", hash: "0000", prevHash: "", isMined: false, isBlinking: false },
        { number: 4, nonce: "89565", data: "", hash: "0000", prevHash: "", isMined: false, isBlinking: false },
        { number: 5, nonce: "89565", data: "", hash: "0000", prevHash: "", isMined: false, isBlinking: false },
        { number: 6, nonce: "89565", data: "", hash: "0000", prevHash: "", isMined: false, isBlinking: false },
    ]);
    ...
    const mineBlock = async (blockIndex:number) => {
        ...
        try {
            const res = await axios.post("/api/hash", {
                input: blocks[blockIndex].data + blocks[blockIndex].number + blocks[blockIndex].nonce + blocks[blockIndex].prevHash,
                encoding: "Hex",
                mine: true,
            });
    ...
    return (
        <div className="container mx-auto px-4">
            <h1 className="text-4xl font-bold text-black text-center mb-4">BlockChain</h1>
            <div className="flex overflow-auto flex-col md:flex-row md:space-x-4">
                {blocks.map((block, index) => (
                    <motion.div
                        key={index}
                        className={`${block.isBlinking ? 'bg-red-300' : 'bg-teal-600'} backdrop-blur-lg rounded-3xl p-6 shadow-xl mb-4 md:mb-0 md:w-96 flex-shrink-0`}
                        animate={{ opacity: block.isBlinking ? [1, 0.5, 1] : 1 }}
                        transition={{ repeat: block.isBlinking ? Infinity : 0, duration: 3 }}
                    >
                        <div className="space-y-6">
                            <div>
                                <label className="block text-xl font-semibold text-white mb-2">Block Number</label>
                                <input
                                    type="number"
                            ...
                            <div>
                                <label className="block text-xl font-semibold text-white mb-2">Nonce</label>
                                <input
                                    type="text"
                                    className="w-full bg-white rounded-xl px-3 py-2 focus:outline-none"
                                    value={block.nonce}
                                    onChange={(e) => handleInputChange(index, 'nonce', e.target.value)}
                                />
                            </div>
                            <div>
                                <label className="block text-xl font-semibold text-white mb-2">Data</label>
                                <input
                                    type="text"
                                    className="w-full bg-white rounded-xl px-3 py-2 focus:outline-none"
                                    value={block.data}
                            ...
                            <div>
                                <label className="block text-xl font-semibold text-white mb-2">Previous Hash</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xl font-semibold text-white mb-2">Hash</label>
                                <input
                                    type="text"
                                    className="w-full bg-white rounded-xl px-3 py-2 focus:outline-none cursor-not-allowed"
                                    value={block.hash}
                                    ...
                                    style={{ backgroundColor: block.hash && !block.hash.startsWith("0000") ? 'rgba(255, 0, 0, 0.2)' : 'rgba(255, 255, 255, 0.2)' }}
                                />
                            </div>

                            <button
                                onClick={() => mineBlock(index)}
                                className={`w-full py-3 rounded-xl text-white font-bold ${block.isMined ? 'bg-green-600' : 'bg-blue-500 hover:bg-blue-600 transition-all'}`}
